...
export class AdminDashboardComponent implements OnInit, OnDestroy {
  ...
  stats      = signal<AppStats>({ totalDoctors: 0, totalPatients: 0, totalConsultations: 0, activeToday: 0 });
  ...
  this.confirmationService.confirm({
    ...
    accept: () => {
      ...
      this.adminService.deleteDoctor(doctor.id)
        .pipe(
          takeUntil(this.destroy$),
        ...
        .subscribe({
          next: () => {
            ...
            this.messageService.add({ severity: 'success', summary: 'Supprimé', detail: 'Le médecin a été retiré de la plateforme.' });
          },
          error: () => {
            this.messageService.add({ severity: 'error', summary: 'Erreur', detail: 'La suppression a échoué. Réessayez.' });
          },
        });
    },
  });
  ...
  saveSettings(): void {
    ...
    this.messageService.add({ severity: 'success', summary: 'Sauvegardé', detail: 'Paramètres mis à jour.' });
